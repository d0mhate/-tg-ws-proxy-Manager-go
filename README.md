...
- это локальный `SOCKS5` или `MTProto` прокси для Telegram
...
3. `15) Secret` → сгенерировать или ввести
4. `16) Public IP` → ввести публичный IP сервера
...
- port: `1080`
- secret: значение из `Advanced → 15) Secret`

Либо использовать готовую ссылку `tg://proxy?server=...&port=...&secret=...` - она отображается в меню после задания Secret и Public IP, и открывает Telegram с уже заполненными полями.
...
MTProto proxy - встроенный режим, при котором клиент Telegram подключается напрямую как к официальному MTProto-прокси, без SOCKS5. Трафик остаётся зашифрован обфускацией MTProto.
...
1. `Advanced`
2. `14) Mode` → выбрать `mtproto`
3. `15) Secret` → сгенерировать случайный или ввести свой (32 hex символа)
4. `16) Public IP` → ввести публичный IP этого сервера

После заполнения Secret и Public IP в строке настроек главного меню появится `tg://proxy?...` ссылка. Пункт `17) Show QR code` становится доступен когда заданы Mode=mtproto, Secret и Public IP.
...
```bash
PROXY_MODE=mtproto MT_SECRET=<32hex> tgm start
PROXY_MODE=mtproto MT_SECRET=<32hex> MT_LINK_IP=1.2.3.4 tgm start
```

Флаги бинарника при прямом запуске:

```bash
./tg-ws-proxy --mode mtproto --secret <32hex> --port 1080
./tg-ws-proxy --mode mtproto --secret <32hex> --link-ip 1.2.3.4 --port 1080
```
