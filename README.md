...
1. `Advanced`
2. `Configure Telegram DC mapping`
...
- **`9) Toggle Cloudflare proxy`** - включает или выключает CF proxy. Когда включён, прокси использует Cloudflare как дополнительный маршрут.
- **`10) Toggle Cloudflare first`** - меняет порядок попыток:
...
Текущее состояние обоих переключателей видно в шапке меню:
...
Для работы CF proxy нужен домен, проксируемый через Cloudflare с поддержкой WebSocket. Встроенный домен доступен сразу без настройки - его достаточно для быстрой проверки.

- **`11) Set Cloudflare domain`** - задать свой домен. Ввести `clear` - вернуть встроенный домен по умолчанию.
- **`12) Check Cloudflare domain`** - проверить домен: manager выполнит WebSocket upgrade к `kws1-kws5` и `kws203` поддоменам и покажет, какие из них отвечают.

Если встроенный домен не работает в вашей сети - зарегистрируйте любой домен, добавьте его в Cloudflare и укажите через пункт `11)`.
...
или через меню: **5) Remove binary and runtime files**
