...
func TestManagerInstallSelectsAarch64ReleaseAssetForOpenWrtFork(t *testing.T) {
	env := managerEnv(t)
	...
	writeFile(t, openwrtRelease, "DISTRIB_ID='RouteRich'\nDISTRIB_ARCH='aarch64_cortex-a53'\n", 0o644)
	writeFile(t, sourceVersion, "v0.0.1\n", 0o644)
	writeFile(t, sourceBin, "#!/bin/sh\necho stale\n", 0o755)
	...
	if !strings.Contains(out, "OpenWrt detected") {
		t.Fatalf("expected OpenWrt detection in output, got:\n%s", out)
	}
